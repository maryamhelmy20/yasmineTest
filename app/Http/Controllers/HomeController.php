...
class HomeController extends Controller
{
    ...
    public function cart_login(Request $request)
    {
        ...
        $is_guest_login = $request->has('guest_login');
        if ($is_guest_login) {
            
            // Handle guest login
            $user = User::create([
                'user_type' => 'customer',
                'temp_user_id' =>$temp_user_id             // Add other required fields for a guest user
            ]);
            // You may want to set the user as logged in, or you can handle it based on your logic
            auth()->login($user, true);
            $yyy= User::where('temp_user_id',$temp_user_id)->get();
            // Redirect the user back
            return back();
        }
        elseif ($request->get('phone') != null) {
   ...
   public function product(Request $request, $slug)
{
    ...
    // Check if $detailedProduct is not null before accessing its 'id' property
    ...
    // Handle the case where $detailedProduct is null or not published
    abort(404);
}

    ...
    <div class="card-body">
        @if (addon_is_activated('club_point'))
            ...
            @foreach ($carts as $key => $cartItem)
                            @php
                    $product = \App\Models\Product::find($cartItem['product_id']);
                $str=$cartItem['variation'];
                $product_stocks=$product->stocks->where('variant',$str)->first();
                if($product_stocks->earn_point!=0)
                {
               

    $total_point += $product_stocks->earn_point * $cartItem['quantity'];
                }
else {

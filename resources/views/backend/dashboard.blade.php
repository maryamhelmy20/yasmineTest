...
<div class="row gutters-10">
    <div class="col-lg-6">
        <div class="row gutters-10">
            ...
            <div class="col-6">
                                <a href="{{route('products.out')}}" class="aiz-side-nav-link">
                <div class="bg-grad-4 text-white rounded-lg mb-4 overflow-hidden">
                    <div class="px-3 pt-3">
                        <div class="opacity-50">
                            <span class="fs-12 d-block">{{ translate('Total') }}</span>
                            {{ translate('Product out of stock') }}
                        </div>
                        <div class="h3 fw-700 mb-3">{{ \App\Models\Product::where('current_stock',0)->count() }}</div>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
                        <path fill="rgba(255,255,255,0.3)" fill-opacity="1" d="M0,128L34.3,112C68.6,96,137,64,206,96C274.3,128,343,224,411,250.7C480,277,549,235,617,213.3C685.7,192,754,192,823,181.3C891.4,171,960,149,1029,117.3C1097.1,85,1166,43,1234,58.7C1302.9,75,1371,149,1406,186.7L1440,224L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z"></path>
                    </svg>
            ...
            <div class="col-6">
                <div class="bg-success  text-white rounded-lg mb-4 overflow-hidden">
                    <div class="px-3 pt-3">
                        ...
                        <div class="h3 fw-700 mb-3">{{ \App\Models\order::sum('grand_total') .' '.'EGP'}}</div>
            ...
            <div class="col-6">
                ...
                <div class="bg-grad-4 text-white rounded-lg mb-4 overflow-hidden">
                    <div class="px-3 pt-3">
                        ...
                        <div class="h3 fw-700 mb-3">{{ \App\Models\Shop::count() }}</div>
                    </div>
                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 1440 320">
                        <path fill="rgba(255,255,255,0.3)" fill-opacity="1" d="M0,128L34.3,112C68.6,96,137,64,206,96C274.3,128,343,224,411,250.7C480,277,549,235,617,213.3C685.7,192,754,192,823,181.3C891.4,171,960,149,1029,117.3C1097.1,85,1166,43,1234,58.7C1302.9,75,1371,149,1406,186.7L1440,224L1440,320L1405.7,320C1371.4,320,1303,320,1234,320C1165.7,320,1097,320,1029,320C960,320,891,320,823,320C754.3,320,686,320,617,320C548.6,320,480,320,411,320C342.9,320,274,320,206,320C137.1,320,69,320,34,320L0,320Z"></path>
                    </svg>
    ...
    <div class="col-lg-6">
        <div class="row gutters-10">
            ...
            <div class="col-6">
                <div class="card">
                    <div class="card-header">
                        <h6 class="mb-0 fs-14">{{ translate('Shiping Order') }}</h6>
                    </div>
                    <div class="card-body">
...
<div class="row gutters-10">
     <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h6 class="mb-0 fs-14">{{ translate('City order') }}</h6>
            </div>
            <div class="card-body">
...
<script type="text/javascript">
    AIZ.plugins.chart('#pie-1',{
        ...
        data: {
            labels: [
                '{{translate('Total published products').' '.  \App\Models\Product::where('published', 1)->count() }}',
                '{{translate('Total sellers products')  .' '.  \App\Models\Product::where('published', 1)->where('added_by', 'seller')->count()                }}',
                '{{translate('Total admin products').' '.  \App\Models\Product::where('published', 1)->where('added_by', 'admin')->count() }}'
            ],
            datasets: [
                {
                    data: [
    ...
    AIZ.plugins.chart('#pie-2',{
        ...
        data: {
            labels: [
                '{{translate('Total sellers') .' '. \App\Models\Shop::count() }}',
                '{{translate('Total approved sellers').' '.\App\Models\Shop::where('verification_status', 1)->count()}}',
                '{{translate('Total pending sellers').' '.\App\Models\Shop::where('verification_status', 0)->count() }}'
            ],
            datasets: [
                {
                    data: [
      ...
      AIZ.plugins.chart('#pie-3',{
        ...
        data: {
            labels: [
              '{{translate('Total sales unpaid').' '.\App\Models\Order::where( 'payment_status','unpaid')->sum('grand_total').' '.'EGP'}}',
                '{{translate('Total sales Shipping').' '.\App\Models\OrderDetail::where( 'payment_status','paid')->sum('shipping_cost').' '.'EGP'}}',
               '{{translate('Total sales paid').' '.\App\Models\Order::where( 'payment_status','paid')->sum('grand_total').' ' .'EGP'}}'
            ...
            datasets: [
                {
                    data: [
                        ...
                        {{ \App\Models\Order::where( 'payment_status','unpaid')->sum('grand_total') }},
                        {{ \App\Models\OrderDetail::where( 'payment_status','paid')->sum('shipping_cost') }},
     ...
     AIZ.plugins.chart('#pie-4',{
        ...
        data: {
            labels: [
    '{{ 'order delivered'.' '.\App\Models\Order::where( 'delivery_status','delivered')->count()  }}',
                '{{' order on The Way'.' '. \App\Models\OrderDetail::where( 'delivery_status','on_the_way')->count()}}',
                '{{'order pending'.' '. \App\Models\Order::where( 'delivery_status','pending')->count()}}',
                '{{' order confirmed'.' '.\App\Models\Order::where( 'delivery_status','confirmed')->count() }}'
            ],
            datasets: [
                {
                    data: [
                       {{ \App\Models\Order::where( 'delivery_status','delivered')->count() }},
                        {{ \App\Models\OrderDetail::where( 'delivery_status','on_the_way')->count() }},
            ...
            datasets: [{
                ...
                data: [
                     @foreach ($citys as $key => $city )
                      ...
                      '{{ \App\Models\Address::where('city_id',$city->id)->count() }}',
                         @endforeach
                ],
                backgroundColor: [
    @foreach ($citys as $key => $city )
